...
use color::{ColorPipeline, ColorSpace, ToneMapper, TransferFunction};
...
fn gpu_pipeline_matches_cpu_reference() {
    ...
    let size = 2;
    let src_data = vec![
        code_px, code_px, code_px, 255, code_px, code_px, code_px, 255, code_px, code_px, code_px,
        255, code_px, code_px, code_px, 255,
    ];
    ...
    for row in 0..size {
        for px_index in 0..size {
            let px =
                &data[((row as usize) * (BYTES_PER_ROW as usize)) + (px_index as usize) * 4..][..4];
